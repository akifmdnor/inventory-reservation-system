...
export function InventoryPanel(props: { availability: Availability | null; productLabel: string }) {
  ...
  if (!availability) {
    return (
      <div className="glass-panel rounded-2xl p-6 text-sm text-iris-muted">
        Select a product to view live stock.
      </div>
    );
  }
  const rows = [
  ...
  return (
    <div className="glass-panel rounded-2xl p-6">
      <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-teal-700">Stock</p>
      <h3 className="mt-3 text-lg font-semibold tracking-tight text-iris-text">{productLabel}</h3>
